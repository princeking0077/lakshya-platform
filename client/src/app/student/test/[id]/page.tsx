...
const TestInterface = () => {
    ...
    const fetchTest = async () => {
        try {
            const res = await axios.get(`${API_BASE_URL}/api/tests/${params.id}`);
    ...
    const handleSubmit = async (auto = false) => {
        ...
        try {
            ...
            router.push(`/student/test/${params.id}/result?score=${result.score}&total=${result.totalMarks}&correct=${result.correctAnswers}&incorrect=${result.incorrectAnswers}`);
        } catch (error) {
            console.error(error);
